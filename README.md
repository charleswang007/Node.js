...
* 同步呼叫會造成呼叫處理程序的 IO 阻塞，非同步呼叫不會造成呼叫處理程序的 IO 阻塞。

* Libuv 是一個跨平台的非同步 IO 庫，它結合了 UNIX 下的 libev 和 Windows 下的 IOCP 的特性，最早由 Node 的作者開發，專門為 Node 提供多平台下的非同步 IO 支援。Node 中的非阻塞 IO 及事件循環的底層機制，是由 libuv 來實現的。

* 單執行緒支援高平行處理，通常都是依靠非同步 + 事件驅動 (循環) 來實現的，非同步使得程式在面臨多個請求時不會發生阻塞，事件循環提供了 IO 呼叫結束後呼叫回呼函數的能力。

* Node 程式雖然在單執行緒中執行，但仍然支援高平行處理，就是依靠事件循環 (Event Loop) 實現的。

* 以 Ajax 請求為例，當 JavaScript 執行到對應的程式時，就為這行程式碼註冊了一個事件，在發出請求後該敘述就執行完畢了，後續的操作會交給回呼函數來處理。此時，瀏覽器背後的循環正在不斷檢查事件佇列，在 Ajax 操作完成之前，事件佇列裡還是空的。如果 Ajax 操作完成了，這個佇列中就會增加一個事件，隨後被循環檢查到，如果這個事件綁訂了一個回呼方法，那麼循環就會去呼叫這個方法。
